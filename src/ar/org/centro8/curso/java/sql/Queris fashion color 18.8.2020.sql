...
-- 3 a stock cero
select * from ropas where stock=0;
...
select id_ropa,ropa,precio_ropa,cantidad,monto From detalles where numero_factura=2;
 -- recuperar el siguiente numero de factura
...
-- 13- cuál es el descuento más uso para facturar
...
SELECT genero_ropa, COUNT( genero_ropa) AS sexo
FROM  ropas
GROUP BY genero_ropa
ORDER BY sexo;

-- 18-b- Ropas de genero f
select ropa,genero_ropa from ropas where genero_ropa= 'femenino';
